...
namespace ProviderImplementation.ProvidedTypes
    ...
    type TypeProviderForNamespaces(config: TypeProviderConfig, namespacesAndTypes: list<(string * list<ProvidedTypeDefinition>)>, assemblyReplacementMap: (string*string) list, sourceAssemblies: Assembly list, addDefaultProbingLocation: bool) as this =
        ...
        do
            if not config.IsInvalidationSupported then
                AppDomain.CurrentDomain.add_AssemblyResolve handler
        //let tpguid = System.Guid.NewGuid().ToString()
        //do
        //    let cfg = 
        //        sprintf "IsInvalidationSupported=%b ResolutionFolder=%s RuntimeAssembly=%s"
        //            config.IsInvalidationSupported config.ResolutionFolder config.RuntimeAssembly 
        //    File.AppendAllLines(@"H:\tplog.txt", [| "Created TP " + tpguid + cfg |])
#endif
        ...
        interface IDisposable with
            member x.Dispose() =
                ...
                //File.AppendAllLines(@"H:\tplog.txt", [| "Disposed TP " + tpguid |])
#endif
